...
languages. With that in mind I placed related modules here.
I hope others will find them useful (and expand upon them).

Different platforms have different vintages of their compilers. In 
addition there are multiple Fortran compilers and various OS configurations
to consider. So in general, the installation may need customized due to
differences such as ...
...
+ DTIO support
+ real128 support
+ cpp/fpp preprocessing
+ preferred compiler options

## Platform
+ POSIX support
+ X11 support
+ MSWindows graphics support
+ developer library support (ncurses curl tinfo sqlite3 readline X11 lua)
+ compiler installation
+ infrastructure conda, wsl, X11, make, cmake, fpm, 
+ package managers for platforms apt, ...
+ IDE support and installation

So in general if you make(1) you would enter
```bash
cd src
make -j 8 -k
```
and remove or edit files or add external packages as needed.

Alternatively if you use fpm(1). look at the fpm.sh script, which shows
several examples of using fpm(1).

Note the fpm.sh script may select a pre-groomed branch for a particular
compiler. The existing branches can be listed using
```bash
git branch -a
* master
  remotes/origin/HEAD -> origin/master
  remotes/origin/flang-posix-nox11
  remotes/origin/flang-posix-x11
  remotes/origin/gfortran-posix-nox11
  remotes/origin/gfortran-posix-x11
  remotes/origin/ifx-posix-nox11
  remotes/origin/ifx-posix-x11
```
So to use gfortran(1) on a Linux box with X11 windows you might start
with
```bash
...
```bash
git switch gfortran-posix-x11
fpm test  --compiler gfortran --flag=-fcoarray=single 
```
If you are interested in the C wrappers included by default you must
have additional libraries available. An example for Ubuntu would be
```bash
sudo apt update
sudo apt install libsqlite3-dev
sudo apt install libreadline-dev
sudo apt install curl
sudo apt install libcurl4-openssl-dev # install the curl developer libraries 
sudo apt install flex
sudo apt install gfortran
sudo apt install make
```
If you do not have these libraries and do not wish to create the interfaces
remove the module source files that fail to compile and fpm(1) and the 
Makefile will compensate for the files being removed or renamed.
