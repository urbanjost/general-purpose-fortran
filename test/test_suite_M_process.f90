...
character(len=4096) :: line            ! long enough to hold any expected line
   call unit_test_start('process_close',msg='')
   string=''
   !!call process_close(fp,ierr)             ! not open yet
   !!call unit_test('process_close', ierr.ne.0, 'close process before opening it',ierr)
   ...
   enddo
   call unit_test_msg('process_close','string=',string )
   call process_close(fp,ierr)             ! Wrap up
   call unit_test('process_close', ierr.eq.0, 'close process ',ierr)
   call process_open_write('cat',fp,ierr)    ! open process to write to that is not terminated
   call process_close(fp,ierr)
   !!call unit_test('process_close', ierr.eq.0, 'close process that is open',ierr)
   call unit_test_done('process_close',msg='')
...
character(len=:),allocatable :: string                                     ! hold results, assuming sufficient memory is available
character(len=4096) :: line                                                ! long enough to hold any expected line
  call unit_test_start('process_open_read',msg='')
  string=''
  call process_open_read('echo a;echo b;echo c',fp,ierr)                   ! open process to read from
  call unit_test('process_open_read', ierr.eq.0, 'open ierr=',ierr)
  do                                                                       ! read output of process till end
     call process_readline(line,fp,ierr)
     if(ierr.ne.0)exit
     string=string//trim(line)//'+'                                        ! append output lines together
  enddo
  call unit_test('process_open_read', string.eq.'a+b+c+', string)
  call process_open_read('echo a;echo b;echo c',fp,ierr)                   ! open process to read from
  call unit_test('process_open_read', string.eq.'a+b+c+', 'open already open process,ierr=',ierr)
  call process_close(fp,ierr)                                              ! Wrap up
  !!call unit_test('process_open_read', ierr.eq.0, 'close ierr=',ierr)
  call unit_test_done('process_open_read',msg='')
...
character(len=256)           :: line
   call unit_test_start('process_open_write',msg='')
   ! clear scratch file
   ...
   call unit_test('process_open_write', ierr.eq.0, 'ierr=',ierr)
   call process_close(fp,ierr)
   call unit_test('process_open_write', ierr.eq.0, 'no error on close, ierr=',ierr)
   ! check expected file
   open(newunit=lun,file='_scratch_.txt')
   read(lun,'(a)',iostat=ios)line
   call unit_test('process_open_write', line.eq.'one', 'line 1:',line)
   read(lun,'(a)',iostat=ios)line
   call unit_test('process_open_write', line.eq.'two', 'line 2:',line)
   close(unit=lun,iostat=ios,status='delete')
   ...
   call unit_test('process_open_write', ierr.ge.0, 'write of "echo three >_scratch_.txt", ierr=',ierr)
   call process_writeline('echo four >>_scratch_.txt',fp,ierr)
   call unit_test('process_open_write', ierr.ge.0, 'write of "echo four >>_scratch_.txt", ierr=',ierr)
   call process_close(fp,ierr)
   call unit_test('process_open_write', ierr.eq.0, 'should now be closed, ierr=',ierr)
   ! check expected file
   open(newunit=lun,file='_scratch_.txt')
   read(lun,'(a)',iostat=ios)line
   call unit_test('process_open_write', line.eq.'three',  'line 1:',line)
   read(lun,'(a)',iostat=ios)line
   call unit_test('process_open_write', line.eq.'four',   'line 2:',line)
   close(unit=lun,iostat=ios,status='delete')
   !
   call unit_test_done('process_open_write',msg='')
...
character(len=:),allocatable :: string
   ...
   call unit_test_start('process_readall',msg='')
   call unit_test('process_readall', string.eq.'A B C', string)
   call unit_test_done('process_readall',msg='')
...
character(len=:),allocatable :: string                                      ! hold results, assuming sufficient memory is available
character(len=4096) :: line                                                 ! long enough to hold any expected line
   call unit_test_start('process_readline',msg='')
   string=''
   call process_open_read('echo a;echo b;echo c',fp,ierr)                   ! open process to read from
   do                                                                       ! read output of process till end
      call process_readline(line,fp,ierr)
      ...
      string=string//trim(line)//'+'                                        ! append output lines together
   enddo
   call unit_test('process_readline', string.eq.'a+b+c+', string)
   call process_close(fp,ierr)                                              ! Wrap up
   call unit_test_done('process_readline',msg='')
...
character(len=256)           :: line
   call unit_test_start('process_writeline_array',msg='')
   ! clear scratch file
   open(newunit=lun,file='_scratch_.txt',iostat=ios)
   close(unit=lun,iostat=ios,status='delete')
   ! start shell
   ...
   call unit_test('process_writeline_array',ierr.ge.0,'wrote four lines, ierr=',ierr)
   call process_close(fp,ierr)
   ! check expected file
   open(newunit=lun,file='_scratch_.txt')
   do i=1,4
      read(lun,'(a)',iostat=ios)line
      if(ios.ne.0)exit
      call unit_test('process_writeline_array',line.eq.lines(i),'got ',line,'expected',line)
   enddo
   close(unit=lun,iostat=ios,status='delete')
   call unit_test('process_writeline_array',i.eq.5,'number of lines',i-1)
   call unit_test_done('process_writeline_array',msg='')
...
character(len=*),parameter   :: lines(*)=[character(len=10) :: 'one','two','three','four']
character(len=256)           :: line
   call unit_test_start('process_writeline_scalar',msg='')
   ! clear scratch file
   open(newunit=lun,file='_scratch_.txt',iostat=ios)
   close(unit=lun,iostat=ios,status='delete')
   ! start shell
   ...
   do i=1,size(lines)
      read(lun,'(a)',iostat=ios)line
      if(ios.ne.0)exit
      call unit_test('process_writeline_scalar',line.eq.lines(i),line)
   enddo
   close(unit=lun,iostat=ios,status='delete')
   call unit_test('process_writeline_scalar',i.eq.5,'number of lines',i)

   call unit_test_done('process_writeline_scalar',msg='')
