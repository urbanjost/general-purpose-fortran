...
use M_framework__verify, only : unit_test, unit_test_good, unit_test_done, unit_test_start, unit_test_level
use :: M_factor
implicit none
   call unit_test_start('least_common_multiple')
   ! SCALAR:
   ...
   call unit_test_done('least_common_multiple')
contains
...
character(len=1024)         :: line
   ...
   call unit_test('least_common_multiple',least_common_multiple(ii,jj).eq.answer,msg=line)
...
integer                     :: i
  ...
  call unit_test('least_common_multiple',least_common_multiple(array).eq.answer,message,line, ']) the value is',answer)
end subroutine writeit_v

end subroutine test_least_common_multiple
!TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT
subroutine test_greatest_common_divisor
use M_framework__verify, only : unit_test, unit_test_good, unit_test_done, unit_test_start
use :: M_factor
implicit none
integer, allocatable :: matrix(:,:)
   call unit_test_start('greatest_common_divisor')
   ! SCALAR:
   ...
   call unit_test_done('greatest_common_divisor')
   ...
   character(len=1024) :: line
      ...
      call unit_test('greatest_common_divisor',greatest_common_divisor(ii,jj).eq.answer,msg=line)
   end subroutine writeit
   ...
   character(len=1024) :: line2
      ...
      call unit_test('greatest_common_divisor',greatest_common_divisor(vector).eq.answer,line1,line2)
   end subroutine writeit_v
   ...
   character(len=1024) :: line3
      ...
      call unit_test('greatest_common_divisor',greatest_common_divisor(matrix).eq.answer,line1,line2,line3)
...
integer,parameter :: nn(*)=[      9973, 1127326861, 1127326883, 1127326897, 1127326901, 1127326927, 1127326957, &
   ...
   call unit_test_start('i_is_prime')
   do i=2,n
   ...
   call unit_test('i_is_prime',icount.eq.1229,msg='should be 1229')
   call unit_test('i_is_prime',isum.eq.5736396,msg='should be 5736396')

   CHECKTHEM: block
   do i=1,size(nn)
      call unit_test('i_is_prime',i_is_prime(nn(i)),'testing',nn(i),'which should be prime')
   enddo
   endblock CHECKTHEM

   call unit_test('i_is_prime', all(.not.i_is_prime([4,6,8,9,10,12,14,15,16,18])),msg='check some values that are not prime')

   call unit_test_done('i_is_prime')
...
integer,allocatable :: exponents(:)
   ...
   call unit_test_start('prime_factors')
   number=2030303
   primes=[11,379,487]
   exponents=[1,1,1]
   call prime_factors(number, nprm, iprm, iexp,verbose)
   call unit_test('prime_factors',all(iprm(:nprm).eq.primes).and.all(iexp(:nprm).eq.exponents),'factors of ',number)
   ...
   call unit_test_done('prime_factors')
