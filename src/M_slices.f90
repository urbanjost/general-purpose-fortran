...
!*==dl_slices.f90 processed by SPAG 8.01RF 03:03 13 Dec 2024
!
...
!
         ...
         if ( alpha<0. .or. alpha>88. .or. beta<1. .or. beta>90. ) then
            write (*,*) '("*** dl_slices INPUT ANGLE ERROR ***") ALPHA=' , alpha , '(allowed 0 to 88) BETA=' , beta , &
                       &'(allowed 1 to 90)'
            if ( allocated(h) ) deallocate (h)
            if ( allocated(p) ) deallocate (p)
            return
...
!
         ...
         if ( iaxis/=0 ) then
            ...
            call vxpt3_(xp,yp,aminq,1,1,nx)
            ...
            call vxpt3_(xp1,yp1,aminq,nx,1,nx)
                                         ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
         ...
         if ( iflag1==2 ) call color_(icol(5))
                                         ! PEN COLOR
!
!     PLOT FRONT PLATE
!
         ...
         if ( beta/=90.0 ) then
            ...
            if ( iflag10/=1 ) then
               ...
               call vxpt3_(xp,yp,aminq,nx,1,nx)
                                    ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
               call draw_(xp,yp)
               do i = 1 , nx - 1
                    ! ADD SIDE LINES
                  call move_(h(i,1),h(i,2))
                  call vxpt3_(xp,yp,aminq,i,1,nx)
                                      ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                  call draw_(xp,yp)
                  call vxpt3_(xp,yp,aminq,i+1,1,nx)
                                        ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                  call draw_(xp,yp)
               enddo
            endif
...
!
            ...
            do i = 1 , nz
               ...
               call vxpt3_(xp,yp,a(nx,i),nx,i,nx)
                                         ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
               h(nx+i,1) = xp
               h(nx+i,2) = yp
               call draw_(xp,yp)
            ...
            if ( iflag10/=1 ) then
               ...
               do i = 2 , nz
                       ! ADD SIDE LINES
                  call vxpt3_(xp2,yp2,aminq,nx,i,nx)
                                             ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                  call draw_(xp2,yp2)
                  call vxpt3_(xp,yp,a(nx,i),nx,i,nx)
                                            ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
...
!        DETERMINE START POINT LOCATION
                  call vxpt3_(xp1,yp1,a(idct,iz),1,iz,nx)
...
!           IPCT=IPCT+1
                     ...
                     do i = 1 , nx
                    ! (VERY RARE)
                        call vxpt3_(xp1,yp1,a(i,iz),i,iz,nx)
                                                 ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                        if ( xp1>h(1,1) ) then
                           idct = i - 1
                           call vxpt3_(dx1,dy1,a(idct,iz),idct,iz,nx)
                                                          ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                           hhigh = .false.
                           hx1 = h(1,1)
                           hy1 = h(1,2)
                           ...
                           idct = idct + 1
                           ihct = ihct + 2
                           call vxpt3_(dx2,dy2,a(idct,iz),idct,iz,nx)
                                                          ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                           idct = idct + 1
                           spag_nextblock_2 = 2
                  ...
                  idct = 2
                  call vxpt3_(dx1,dy1,a(1,iz-1),1,iz-1,nx)
                                               ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                  call vxpt3_(dx2,dy2,a(1,iz),1,iz,nx)
                                               ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
...
!
                     if ( hx2<=dx2 ) then
                        ...
                        if ( ihct>ihold+1 ) then
                           do
                              if ( idct<=nx+1 ) then
                                 call vxpt3_(x,y,a(idct-1,iz),idct-1,iz,nx)
                                                          ! DETERMINE X,Y VALUE OF A POINT ON 3-D SURFACE
                                 if ( ipct>maxsize_local ) then
                                    spag_nextblock_1 = 3
                                    cycle spag_dispatchloop_1
...
subroutine dl_symbol(x,y,s,t,a,nn,is)
!*==dl_symbol.f90 processed by SPAG 8.01RF 03:03 13 Dec 2024
...
!*==axisa_.f90 processed by SPAG 8.01RF 03:03 13 Dec 2024
!
...
!*==axisb_.f90 processed by SPAG 8.01RF 03:03 13 Dec 2024
!
...
!*==number_.f90 processed by SPAG 8.01RF 03:03 13 Dec 2024
!
...
!*==plot_.f90 processed by SPAG 8.01RF 03:03 13 Dec 2024
!
...
!     DRAW LINE SEGMENT  ISELECT0=2,3 (and -2,-3)
   ...
   if ( ior(ivta,ivtb)/=0 ) then        ! LINE ENTIRELY VISIBLE
      ...
      if ( ivta/=0 ) call clipit_(ivta,xtemp,ytemp,xlastscaleq,ylastscaleq,viewportq(1),viewportq(2),viewportq(3),viewportq(4))
      xlastscaleq = xcon
      ylastscaleq = ycon
      if ( ivta/=0 ) return
      xcon = xtemp
...
!*==range_.f90 processed by SPAG 8.01RF 03:03 13 Dec 2024
!
