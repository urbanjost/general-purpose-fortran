...
'   such a reduction as argument. If A is an array, the reduction is', &
...
'              which shall be nonpolymorphic and have the same type', &
'             and type parameters as A. The function shall return a', &
'             nonallocatable scalar of the same type and type parameters', &
'             as A. The function shall be the same on all images and', &
'             with regards to the arguments mathematically commutative', &
'             and associative. Note that OPERATION may not be an elemental', &
'             function, unless it is an intrinsic function.  result_image', &
'             - (optional) a scalar integer expression; if present, it', &
'             shall have the same the same value on all images and refer', &
'             to an image of the current team.', &
'', &
'  STAT       (optional) a scalar integer variable', &
'', &
'  ERRMSG     (optional) a scalar character variable', &
...
'', &
'DESCRIPTION', &
'   The elemental function MERGE(3f) selects values from two arrays', &
'   or scalars according to a logical mask. The result is equal to an', &
'   element of TSOURCE where the corresponding element of MASK is .true.,', &
'   or an element of FSOURCE when it is .false. .', &
'', &
...
'   divide by zero being generated; as 1.0/x may be evaluated for all', &
'   values of x before the mask is used to select which value to retain:', &
'', &
'      y = merge( 1.0/x, 0.0, x /= 0.0 )', &
'', &
'   Note the compiler is also free to short-circuit or to generate an', &
'   infinity so this may work in many programming environments but is', &
'   not recommended.', &
'', &
