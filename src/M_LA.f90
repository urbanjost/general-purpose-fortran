...
!             mat_rrotg,mat_wnrm2
!     FORTRAN abs,DIMAG,DMAX1
!     FORTRAN MAX0,MIN0,MOD,sqrt
!
!     INTERNAL VARIABLES
!
...
!           CALCULATE THE SHIFT.
!
                  scale = dmax1(abs(sr(m)),abs(sr(m-1)),abs(er(m-1)),abs(sr(l)),abs(er(l)))
                  sm = sr(m)/scale
                  smm1 = sr(m-1)/scale
                  emm1 = er(m-1)/scale
                  sl = sr(l)/scale
