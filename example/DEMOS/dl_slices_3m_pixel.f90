          ...
          !                 JSU,                 19-JUL-2005
           ...
           implicit none
             ...
             integer :: nz,nzt
              ...
              !      fill some arrays with data we can plot do j=1,ix do i=1,iz
                     surfdat(j,i)=cos(tpi*real(j-1)/12.0)*cos(tpi*real(i-1)/12.0) enddo enddo

                     call prefsize(501,501) call vinit()

                     call dl_init(12.5,12.5,1.5,1.5,1.0)
                            ! set up plotting surface scale call linewidth(3) call color(4)
              ...
              ! XAXIS:
                     ...
                     xs=-10.0
                            ! xs,xe (R): starting and ending values displayed on x axis xe=10.0 !-----------------------
              ...
              !      > 0 auto exponent scaling (x10 to power) enabled
                     ...
                     smx=0.1
                            ! (R): major tick length on x axis !-----------------------

                     xt='dl_slices X TITLE' ! xt
                            (C): title of x axis (width)

                     nxt=len_trim(xt)
                            ! nxt   (i): number of characters in xt ;nxt = 0 : no axis plotted ; nxt > 0 : normal

              ! YAXIS:

                     ys=-10.0
                            ! ys,ye (R): starting and ending values displayed on y axis ye=10.0 !-----------------------
                     ...
                     tsy=-0.15
                            ! (R): size of title and numbers on y axis
                     ...
                     yt='dl_slices Y TITLE' ! yt
                            (C): title of y axis (width)

                     nyt=len_trim(yt)
                            ! nyt   (i): number of characters in xt ;nyt = 0 : no axis plotted ; nyt > 0 : normal
              ...
              ! ZAXIS: zs=1.0
                     ...
                     zt='SLICE'
                            ! zt    (C): title of z axis (width)

                     nzt=len_trim(zt)
                            ! nzt   (i): number of characters in xt ;nzt = 0 : no axis plotted ; nzt > 0 : normal
              ...
              !      iax < 0 or mod(iflag,10)=1)
                     ...
                     dm=-1.0
                            ! dm,dx (R): minimum and maximum values of SURFDAT array dx=1.0 ! view angles
              ...
              !      z axis appears to run into the screen but is angled to the right iframe=1 b=15.0
                     ...
                     do i10=1,85
                            ! Animate cycling thru angle A a=i10 call color(7) call clear() call color(0)
              ...
              !      add a label after master routine call call color(1) call linewidth(1) call
                     dl_symbol(0.0,0.0,0.25,'VAX3DX',0.0,6,-1) movie(iframe,:,:)=p_pixel(:,:) iframe=iframe+1 enddo

                     a=25

                     do i20=1,90
                            ! Animate cycling thru angle B b=i20 call color(7) call clear() call color(0)
              ...
              & xt,nxt,
                     ...
                     & zt,nzt,
                            & zs,ze,nmz,nnz,mlz,tsz,ndz,smz,dm,dx,icol) movie(iframe,:,:)=p_pixel(:,:) iframe=iframe+1 enddo

              iax=01 iflag=012 ii=1

              do i40=1,90*ii
                     ! Animate cycling thru angles A and B a=real(i40)/ii/2.0 ! should get warning when this exceeds 85
                     b=real(i40)/ii/2.0 call color(7) call clear() call color(0)
              ...
              call vexit()
                     ! close up plot package call write_animated_gif('dl_slices.3m_pixel.gif',movie,p_colormap,delay=5) !call
                     execute_system_command('display dl_slices.3m_pixel.gif') end program demo_dl_slices

                                                         August 30, 2021                                            dl_slices(3)
