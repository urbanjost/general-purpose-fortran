program testit
   ...
   use M_CLI2,    only : set_args, sget, lget, filenames=>unnamed
   ...
   call set_args('--separator:s "::"', help_text=[ CHARACTER(LEN=128) :: &
   'NAME                                                                           ',&
   '    lineup(1f) - [APPS] align strings in Fortran declarations                  ',&
   '    (LICENSE:PD)                                                               ',&
   '                                                                               ',&
   'SYNOPSIS                                                                       ',&
   '    lineup [--separator string][filename(s) ][ --help| --version]              ',&
   '                                                                               ',&
   'DESCRIPTION                                                                    ',&
   '    A simple filter that finds and aligns a string (defaults to "::") in a     ',&
   '    block of text. A stylistic preference for Fortran declaration statements   ',&
   '                                                                               ',&
   'OPTIONS                                                                        ',&
   '   -s, --separator  string to align on. Defaults to "::"                       ',&
   ...
   do ii=1,size(filenames)
      ...
      do i=1,size(pageout)
         ...
         if( j.ge.2)then
            ...
            write(*,'(*(g0))')stretch(left,imax-1,' '),separator,trim(pageout(i)(j+2:))
         else
