...
!                 See "http://www.urbanjost.altervista.org/LIBRARY/libGPF/arguments/src/M_kracken.HTML
!===================================================================================================================================
!  Next ...
...
use M_system, only  : system_islnk, system_stat, system_remove, system_getenv
use M_time, only    : now
   implicit none
  ...
  !integer,public                       :: G_ihelp=ERROR_UNIT             ! output unit for help text
   ...
   character(len=:),allocatable         :: G_header
...
!-----------------------------------------------------------------------------------------------------------------------------------
  ...
  case('DEFINE','INCLUDE','PRINTENV','DOCUMENT','SHOW','STOP')
   ...
   case('HELP','OUTPUT','ERROR','IDENT','@(#)','FILTER','BLOCK')
   case('DEBUG','DISPLAY','ASSERT')
   case(' ')
...
!-----------------------------------------------------------------------------------------------------------------------------------
   if(G_system_on)then
      ...
      if(icmd.ne.0)then                                                             ! if system command failed exit program
         call stop_ufpp('*ufpp:exe* ERROR(c) - SYSTEM COMMAND FAILED: ERROR '//v2s(icmd)//':'//trim(sstat)//':'//trim(command))
      endif
   else
...
!()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()!
!===================================================================================================================================
subroutine call_assert(options)                         !@(#)call_assert(3f): insert call to ASSERT(3f)
character(len=*)             ::  options                ! input line with leading $VERB removed
   ! assumes filename does not have " characters, filename short enough to fit on output line, same with options
   if(G_build_debug_version)then
      if(len_trim(G_file_dictionary(G_iocount)%filename)+len_trim(options).lt.120)then
         call write_out(str( 'call assert(','"'//trim(G_file_dictionary(G_iocount)%filename)//'",', &
         & G_file_dictionary(G_iocount)%line_number,',', options, ')'))
      else
         call write_out(str('call assert("',G_file_dictionary(G_iocount)%filename,'", &'))
         call write_out(str('  &',G_file_dictionary(G_iocount)%line_number,', &'))
         call write_out(str('  &',options,')'))
      endif
   endif
end subroutine call_assert
!===================================================================================================================================
!()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()()!
!===================================================================================================================================
...
! CHECK COMMAND SYNTAX
   ...
   elseif(G_outtype.eq.'version')then  ! if in 'version' mode wrap up the routine
      ...
      write(G_iout,'(a)')"'']"
      write(G_iout,'(a)')"   WRITE(*,'(a)')(trim(help_text(i)(5:len_trim(help_text(i),kind=kind(1))-1)),i=1,size(help_text))"
...
!-----------------------------------------------------------------------------------------------------------------------------------
   case('HELP')
      ...
      write(G_iout,'(a)')'! @(#)help_usage(3f): prints help information'
...
!-----------------------------------------------------------------------------------------------------------------------------------
   case('VERSION')
      ...
      write(G_iout,'(a)')'! @(#)help_version(3f): prints version information'
   ...
   if(ilength.ne.0.and.G_MAN.ne.''.and.G_MAN_FILE.ne.' ')then ! if $FILTER ... -file FILE is present generate file in directory/doc
      ...
      if(.not.G_allow_links)then
         if(system_islnk(filename))then
            call stop_ufpp('*ufpp: ERROR - '//trim(filename)//' must not be a softlink')
         endif

         call system_stat(filename,stat_values,ierr)
         if(stat_values(4).gt.1)then
            call stop_ufpp('*ufpp: ERROR - '//trim(filename)//' must not have multiple links')
         endif
      endif
...
'                for use with the what(1) command. This generates an             ',&
...
'   was processed by ufpp(1), the line number will be the linenumber             ',&
'   of the file processed by ufpp, and everything else on the $ASSERT            ',&
'   directive is passed directly to ASSERT(3f). Note that the first              ',&
'   parameter passed on $ASSERT must be a logical expression.                    ',&
'                                                                                ',&
'   The ASSERT(3f) procedure must be in the load path and the "call assert"      ',&
'   must be written to the ufpp(1) output file in a scope where                  ',&
'                                                                                ',&
...
'   the environment variable DEBUGVERSION=T and the -debug switch is             ',&
'   present on the ufpp.                                                         ',&
'                                                                                ',&
...
'                                                                                ',&
'     LANGUAGE                                                                   ',&
'     fortran   If the commandline option -ident is set                          ',&
'                                                                                ',&
...
'@(#)AUTHOR:         John S. Urban>',&
'@(#)REPORTING BUGS: http://www.urbanjost.altervista.org/>',&
'@(#)COMPILED:       2025-06-29 07:10:29 UTC-240>',&
'']
...
!----------------------------------------------------------------------------------------------------------------------------------=
   ...
   case('write')                                ! convert string to a Fortran write statement to unit "IO"
      ...
      chunk=buff
      if(len_trim(buff).gt.chunkmax)then
         write(G_iout,'(a)',advance='no')chunk(:chunkmax)
         write(G_iout,'(a)')'&'
         chunk=buff(chunkmax+1:)
...
!-----------------------------------------------------------------------------------------------------------------------------------
   G_write=.not.lget('ufpp_q')                          ! check if quiet mode is selected to suppress output till a true condition
   G_write0=G_write
   !!G_condop(0)=G_write
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
   ...
   G_header=sget('ufpp_header')                            ! text to output at top of output file
   G_ident=lget('ufpp_ident')                              ! write IDENT as comment or CHARACTER variable
   G_allow_links=lget('ufpp_allow_links')                  ! allow formatting comments for particular post-processors
   if(G_header.eq.'')then
      G_header=system_getenv('UFPP_HEADER')
   endif
!-----------------------------------------------------------------------------------------------------------------------------------
...
!-----------------------------------------------------------------------------------------------------------------------------------
   if(G_header.ne.'')then
      call write_out(trim(G_header))                       ! optional first line to output
