...
use M_verify, only: unit_test,unit_test_good,unit_test_bad,unit_test_done,unit_test_start,unit_test_msg,unit_test_level
!!use M_time, only : ephemeris, fmtdate
implicit none
...
call unit_test_start('ephemeris')

     do planet=1,8
        if(planet.eq.3)cycle
        call ephemeris(itime, planet, D_d,D_m,D_c, A_h,A_m)
        call unit_test('ephemeris',D_c.eq.'S','Compass direction S')
        select case(planet)
        case(1); call unit_test('ephemeris',all([A_h,A_m,D_d,D_m].eq.[21,51,14,45]),'Mercury')
        case(2); call unit_test('ephemeris',all([A_h,A_m,D_d,D_m].eq.[20,26,14,57]),'Venus')
        case(3);
        case(4); call unit_test('ephemeris',all([A_h,A_m,D_d,D_m].eq.[13,08,03,45]),'Mars')
        case(5); call unit_test('ephemeris',all([A_h,A_m,D_d,D_m].eq.[14,32,13,30]),'Jupiter')
        case(6); call unit_test('ephemeris',all([A_h,A_m,D_d,D_m].eq.[13,22,05,42]),'Saturn')
        case(7); call unit_test('ephemeris',all([A_h,A_m,D_d,D_m].eq.[16,11,20,54]),'Uranus')
        case(8); call unit_test('ephemeris',all([A_h,A_m,D_d,D_m].eq.[17,46,22,07]),'Nepture')
        end select

     enddo

call unit_test_done('ephemeris')
...
'@(#)AUTHOR:         John S. Urban>',&
'@(#)COMPILED:       2025-06-29 08:24:23 UTC-240>',&
'']
