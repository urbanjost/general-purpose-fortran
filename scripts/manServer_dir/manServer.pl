...
# Copyright (C) 2001 Rolf Howarth, rolf@squarebox.co.uk
# Released under BSD license, see attached LICENSE document. No warranty.
# Version 1.08  13 Aug 2009
...
sub man2html
{
	...
	if (! -f $file)
	{
		if (-f "$file.gz")
		{
			$zfile = "$file.gz";
			$zcat = "/usr/bin/gzcat";
			$zcat = "/usr/bin/zcat" unless (-x $zcat);
